...
const Product = () => {
  ...
  // console.log("vendorsaa:", vendors);
  // const vendorName = vendor[0].vendor_name;
  // console.log("vendorProd", vendorProd);
  ...
  const columns = [{ name: "Stock Name" }, { name: "Qty Stocked" }, { name: "Qty Remaining" }, { name: "Price" }, { name: "Action" }];
  ...
  useEffect(() => {
    const filter = vendors.filter((data: any) => {
      if (data.vendor_entity_id === Number(router.id)) {
        return data;
      }
    })[0];
  ...
  console.log("datafilter1", vendor);
            ...
            <div className="-mx-4 sm:-mx-8 px-4 sm:px-8 py-4 overflow-x-auto">
              <div className="inline-block min-w-full shadow rounded-lg overflow-hidden">
                <table className="min-w-full leading-normal">
                  ...
                  <tbody>
                    {(vendor.vendor_products || []).map((prodData: any) => (
                      <>
                        <tr>
                          ...
                          <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                            <p className="text-gray-900 whitespace-no-wrap">{prodData.stock?.stock_name}</p>
                          </td>
                          <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                ...
                <div className="px-5 py-5 bg-white border-t flex flex-col xs:flex-row items-center xs:justify-between">
                  ...
                  <div className="inline-flex mt-2 xs:mt-0">
                    ...
                    &nbsp; &nbsp;
                    <button className="text-sm text-indigo-50 transition duration-150 hover:bg-indigo-500 bg-indigo-600 font-semibold py-2 px-4 rounded-r" onClick={() => setCurrentPage(currentPage + 1)} disabled={vendor.length - 1 <= 0}>
                      Next
                    </button>
                  </div>
                </div>
