...
import { BsFillPencilFill, BsThreeDotsVertical, BsTrashFill, BsFillCloudUploadFill, BsClipboardPlusFill } from "react-icons/bs";
...
export default function Index(props: any) {
  ...
  const columns = [{ name: "Vendor" }, { name: "Status" }, { name: "Priority" }, { name: "Register At" }, { name: "Web Url" }, { name: "Action" }];
  ...
  return (
    <div className="bg-white">
      <>
        ...
        <div className="bg-white p-8 rounded-md w-full">
          <div className=" flex items-center justify-between pb-6">
            <div>
              <h2 className="text-gray-600 font-semibold">Data Vendor</h2>
              <span className="text-xs">All vendor</span>
            </div>
            ...
            <div className="-mx-4 sm:-mx-8 px-4 sm:px-8 py-4 overflow-x-auto">
              <div className="inline-block min-w-full shadow rounded-lg overflow-hidden">
                <table className="min-w-full leading-normal">
                  ...
                  <tbody>
                    {(vendors || []).map((vendorData: any) => (
                      <>
                        <tr key={vendorData.vendor_entity_id}>
                          ...
                          <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                            <p className="text-gray-900 whitespace-no-wrap">{vendorData.vendor_name}</p>
                          </td>
                          <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">{vendorData.vendor_active === "1" ? <p className="text-green-500 font-bold whitespace-no-wrap">Active</p> : <p className="text-red-500 font-bold whitespace-no-wrap">Inactive</p>}</td>

                          <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">{vendorData.vendor_priority === "1" ? <p className="text-green-500 font-bold whitespace-no-wrap">Highest</p> : <p className="text-red-500 font-bold whitespace-no-wrap">Lowest</p>}</td>

                          <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                            <p className="text-gray-900 whitespace-no-wrap">
                              {new Date(vendorData.vendor_register_date).toLocaleDateString("en-GB", {
                                day: "numeric",
                                month: "short",
                                year: "numeric",
                              })}
                          ...
                          <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                            <p className="text-gray-900 whitespace-no-wrap">{vendorData.vendor_weburl}</p>
                          </td>
                          {/* <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                            <p className="text-gray-900 whitespace-no-wrap">
                              ...
                              <Transition as={Fragment} enter="transition ease-out duration-100" enterFrom="transform opacity-0 scale-95" enterTo="transform opacity-100 scale-100" leave="transition ease-in duration-75" leaveFrom="transform opacity-100 scale-100" leaveTo="transform opacity-0 scale-95">
                                <Menu.Items className="absolute right-0 mt-2 w-56 origin-top-right divide-y divide-gray-100 rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none z-10">
                                  ...
                                  <div className="px-1 py-1 ">
                                    <Menu.Item>
                                      {({ active }) => (
                                        <button className={`${active ? "bg-violet-500 text-white" : "text-gray-900"} group flex w-full items-center rounded-md px-2 py-2 text-sm`} onClick={() => editOpen(vendorData.vendor_entity_id, vendorData)}>
                                  ...
                                  <div className="px-1 py-1">
                                    <Menu.Item>
                                      {({ active }) => (
                                        <button className={`${active ? "bg-violet-500 text-white" : "text-gray-900"} group flex w-full items-center rounded-md px-2 py-2 text-sm`} onClick={() => deleteOpen(vendorData.vendor_entity_id)}>
                                  ...
                                  <div className="px-1 py-1">
                                    <Menu.Item>
                                      {({ active }) => (
                                        <Link href={`/purchasing/vendor/product/${vendorData.vendor_entity_id}`}>
                                          <button className={`${active ? "bg-violet-500 text-white" : "text-gray-900"} group flex w-full items-center rounded-md px-2 py-2 text-sm`}>
                                            {active ? <BsClipboardPlusFill className="mr-2 h-5 w-5 " aria-hidden="true" /> : <BsClipboardPlusFill className="mr-2 h-5 w-5 " aria-hidden="true" />}
                                            Add Item Product
                                          </button>
                                        </Link>
                                      )}
                ...
                <div className="px-5 py-5 bg-white border-t flex flex-col xs:flex-row items-center xs:justify-between">
                  ...
                  <div className="inline-flex mt-2 xs:mt-0">
                    ...
                    &nbsp; &nbsp;
                    <button className="text-sm text-indigo-50 transition duration-150 hover:bg-indigo-500 bg-indigo-600 font-semibold py-2 px-4 rounded-r" onClick={() => setCurrentPage(currentPage + 1)} disabled={vendors.length - 1 <= 0}>
                      Next
                    </button>
                  </div>
                </div>
      ...
      {isOpen ? <AddVendor isOpen={isOpen} closeModal={() => setIsOpen(false)} /> : null}
      {isEdit.status ? (
        <EditVendor
          dataVendor={isEdit.data}
          isEdit={isEdit}
          closeModal={() =>
            setIsEdit((prev) => {
