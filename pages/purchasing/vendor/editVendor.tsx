...
import { doUpdateVendor } from "@/redux/purchasing/action/vendorActionReducer";
...
export default function EditVendor(props: any) {
  ...
  const [data, setData] = useState<any>(props.dataVendor);
  ...
  const handleSave = async (data: FormValues) => {
    try {
      ...
      dispatch(doUpdateVendor(props.isEdit.id, data));
      props.closeModal();
    } catch (error) {
      console.error(error);
    }
  ...
  console.log("Datavendor;", props.dataVendor);
  return (
    <div>
            ...
            <div className="flex min-h-full items-center justify-center p-4 text-center">
              <Transition.Child as={Fragment} enter="ease-out duration-300" enterFrom="opacity-0 scale-95" enterTo="opacity-100 scale-100" leave="ease-in duration-200" leaveFrom="opacity-100 scale-100" leaveTo="opacity-0 scale-95">
                <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                  <Dialog.Title className="text-lg font-medium leading-6 text-gray-900">Edit Vendor</Dialog.Title>
                  <div className="mt-4">
                    <form onSubmit={handleSubmit(handleSave, handleError)}>
                      <div className="mb-4">
                        <label className="block text-gray-700 font-bold mb-2">Nama Vendor</label>
                        <input className="shadow appearance-none border rounded w-full py-3 px-1 text-gray-700 leading-tight focus:outline-none focus:shadow-outline " id="name" type="text" defaultValue={data?.vendor_name ?? ""} {...register("vendor_name", registerOptions.vendor_name)} />
                      ...
                      <div className="mb-4">
                        <label className="block text-gray-700 font-bold mb-2">WebUrl</label>
                        <input className="shadow appearance-none border rounded w-full py-3 px-1 text-gray-700 leading-tight focus:outline-none focus:shadow-outline " id="price" type="text" defaultValue={data?.vendor_weburl ?? ""} {...register("vendor_weburl", registerOptions.vendor_weburl)} />
