...
const Detail = () => {
  ...
  useEffect(() => {
    const filter = orders.filter((data: any) => {
      if (data.pohe_id === Number(router.id)) {
        return data;
      }
    })[0];
    if (filter) {
      ...
      <>
        ...
        <div className="bg-white p-8 rounded-md w-full">
          <div className=" flex items-center justify-between pb-6">
            <div>
              <h1 className="text-gray-600 font-semibold">{order.pohe_number}</h1>
              <span className="text-gray-600 font-semibold">
                {" "}
                {new Date(order.pohe_order_date).toLocaleDateString("en-GB", {
                  day: "numeric",
                  month: "short",
                  year: "numeric",
                })}
              ...
              <div className="inline-block min-w-full shadow rounded-lg overflow-hidden">
                ...
                <div className="px-5 py-5 bg-white border-t flex flex-col xs:flex-row items-center xs:justify-between">
                  ...
                  <div className="inline-flex mt-2 xs:mt-0">
                    ...
                    &nbsp; &nbsp;
                    <button className="text-sm text-indigo-50 transition duration-150 hover:bg-indigo-500 bg-indigo-600 font-semibold py-2 px-4 rounded-r" onClick={() => setCurrentPage(currentPage + 1)} disabled={order.length - 1 <= 0}>
                      Next
                    </button>
                  </div>
                </div>
