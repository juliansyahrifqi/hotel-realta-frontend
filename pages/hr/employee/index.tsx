...
export default function Employee() {
  ...
  const totalArr = Array.apply(null, Array(employees?.meta?.total_pages)).map(function (x, i) {
    return i;
  });
  ...
  const deleteOpen = async (id: number) => {
    ...
    console.log("dapetID", id)
  };

  useEffect(() => {
    dispatch(doGetEmployee(page, limit, search, status));
  }, [dispatch, page, limit, refresh, search, status]);
  return (
    <div className="relative overflow-hidden">
      <form className="flex items-center">
        ...
        <div className="relative w-44 ml-2">
          ...
          <select
            ...
            onChange={(e) => (e.target.value === "ALL" ? setStatus("") : setStatus(e.target.value))}>
      ...
      {/* batas atas pagination */}
      {isOpen ? <CreateEmployee isOpen={isOpen} closeModal={() => setIsOpen(false)} /> : null}
