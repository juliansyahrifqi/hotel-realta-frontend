...
export default function DepartmentHistory() {
  ...
  useEffect(() => {
    ...
    setIsDeptHistory(filterDept);
  }, [refresh]);
  console.log("cekDept", router.id);
            ...
            <tbody>
              {(deptHistory.employee_department_histories || []).map((dt: any, index: number) => (
                <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600" key={dt.emp_id}>
                  ...
                  <td className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">{new Date(dt.edhi_end_date).toLocaleDateString("en-GB", {
                    day: "numeric",
                    month: "long",
                    year: "numeric",
                  })}</td>
